...
Extracted from a production Polaris model that needed exactly this kind of uneven, per-level-factor hierarchy (session notes, where ingested, live under the sourcing customer's own `wiki/sources/`).
...
A staging module (`LO 01. ...`) holds the raw N-column CSV (Code Level k, Name Level k, Factor Level k for every k) and computes per-row validation booleans (`Level k complete?`). Imports then create/update each `X_Lk` list from the matching staging columns; finally `Combi 01. Create X hierarchy (complete)` assembles `X Ragged` from the per-level lists. The `Create/Update X Ragged?` line item on each `IM 0k.` decides which rows to push.
...
In practice both directions are needed: a calculation module typically uses the upstream product for cost roll-up, while a reporting module uses the downstream/cascading-cumulate product for re-projection at any chosen level.
...
…and surface "the cumulative factor for *my own level*" without an N-deep IF chain.

## Engine notes

- Designed for **Polaris** — the composite list (`X Ragged`) crossed with multiple time-and-org dimensions yields multi-billion-cell intersections that are intractable on Classic. In a production instance, one calculation module reached 5 billion cells and an input module reached 3 billion — intersections at that scale are Polaris-only. See [[wiki/patterns/planual/02-engine|Planual Ch.2 — Polaris]].
...
4. **Cross-cutting LOOKUPs into deprecated modules.** A calculation module can quietly depend on a property read from a module tagged `DEL` (marked for deletion) — deleting that module would silently break the calc. Trace inbound references before deleting any `DEL`-prefixed module.
